...
{
    public class ShowActionsUtilityCommand : CallbackCommand
    {
        private readonly DbUserService _dbUserService;
        private readonly DbActivitiesService _dbActivitiesService;
        public override string Name => "showActionsUtilityCommand";

        public ShowActionsUtilityCommand(DbUserService dbUserService, DbActivitiesService dbActivitiesService) : base(dbUserService)
        {
        ...
        public override async Task execute(CallbackQuery query, TelegramBotClient client)
        {
            ...
            {
                ikm.Add(InlineKeyboardButton.WithCallbackData(a.Name, "showActionsCommand?name=" + a.Name));
            });
            var markup = new InlineKeyboardMarkup(ikm);
            await client.SendTextMessageAsync(chatId, "Choose activity to show:", replyMarkup: markup);
        }
    }
}
